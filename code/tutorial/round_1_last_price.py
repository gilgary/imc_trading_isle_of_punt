...
class Trader:
    ...
    def get_fair_mid(
        ...
        if len(order_depth.sell_orders) != 0 and len(order_depth.buy_orders) != 0:
            ...
            for price_ask, volume in order_depth.sell_orders.items():
                for price_buy, volume in order_depth.buy_orders.items():
                    ask_volume = abs(order_depth.sell_orders[price_ask])
                    bid_volume = order_depth.buy_orders[price_buy]
                    if ask_volume == bid_volume or abs(ask_volume - bid_volume) == 5:
                        # Found a matching price; set mid_price as this price level
                        mid_price = (price_ask + price_buy) / 2
                    else: 
    ...
    def squid_fair_value(self, product: str, order_depth: OrderDepth, traderObject) -> float:
        if len(order_depth.sell_orders) != 0 and len(order_depth.buy_orders) != 0:
            ...
            if traderObject.get(f"{product}_last_price") is not None:
                last_price = traderObject[f"{product}_last_price"]
                # If the new fair price deviates more than 20% from the last recorded price, default to last price.
                if fair < last_price * 0.8 or fair > last_price * 1.2:
                    fair = last_price
    ...
    def run(self, state: TradingState):
        ...
        if Product.RAINFOREST_RESIN in self.params and Product.RAINFOREST_RESIN in state.order_depths:
            ...
            result[Product.RAINFOREST_RESIN] = (
                resin_take_orders  + resin_make_orders +resin_clear_orders
                #+ amethyst_clear_orders
            )
        ...
        if Product.KELP in self.params and Product.KELP in state.order_depths:
            ...
            result[Product.KELP] = (
                kelp_take_orders + kelp_make_orders + kelp_clear_orders
            )
        ...
        if Product.SQUID_INK in self.params and Product.SQUID_INK in state.order_depths:
            ...
            result[Product.SQUID_INK] = (
                squid_take_orders + squid_ink_make_orders + squid_clear_orders
            )
            #+ starfruit_clear_orders 
